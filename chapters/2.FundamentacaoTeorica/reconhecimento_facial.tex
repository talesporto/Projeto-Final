...
\label{ref:viola-jones}
	...
	\textit{haar} básicas. Para computa-las a imagem integral é usada. Os
	algoritmos \textit{AdaBoost} são usados para aprendizado dentro dos
	classificadores fracos. E por fim os classificadoes em cascatas fecham a
	detecção.
%Figura~\ref{fig:edsonma}:
	...
	Existem inúmeros motivos para se usar as características retangulares, sendo
	que um dos principais é que sistemas baseados em características são muito
	mais rápidos que sistemas baseados em \textit{pixels}~\cite{violajones}.
	A detecção facial em imagens, baseada em simples características retangulares,
	é exemplificada na Figura~\ref{haarfeatures}. 
	...
	Figura \ref{haarfeaturestypes}, ou seja, características com dois, três ou
	quatro retângulos~\cite{violajones}. A presença de uma característica em uma imagem é
	...
	passe por todos, ou quase todos, os classificadores. Os filtros em cada nível são
...
\label{sec:reconhecimento}
	...
	entre dois pontos $P_1$ e $P_2$ em duas dimenções, é dada pela Equação
	...
	Imagens possuem ``ruídos'' e vamos defini-los como qualquer coisa que
	...
	\begin{figure}[htb]
		...
		(b) As seis primeiras componentes principais~\cite{hewitt}. (Adaptada)}
	\end{figure}
