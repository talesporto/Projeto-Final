...
\subsubsection{Detecção de entidades}
...
\subsubsection{Rastreamento de entidades}
	...
	\begin{itemize}
		\item \textbf{Rastreamento de pontos:} as entidades detectadas a cada frame são representadas por pontos e a associação entre os pontos é feita com base no estado anterior da entidade que pode incluir a posição e o movimento. Essa abordagem requer um mecanismo externo para detectar a entidade a cada \textit{frame}~\cite{yilmaz};

		\item \textbf{Rastreamento de núcleo}: as entidades são rastreados pelo cálculo do movimento do núcleo em \textit{frames} consecutivas. Esse movimento geralmente esta na forma de transformações paramétricas como translação e rotação. O núcleo se refere ao formato ou aparência da entidade~\cite{yilmaz}.
