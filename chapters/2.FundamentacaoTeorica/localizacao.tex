...
\label{sec:luz-estruturada}
	...
	\begin{enumerate}
		...
		entidade estimando sua distância a múltiplos ponto de referência. Calcular a
		posição de uma entidade em duas dimensões requer estimativas de distância de
		três pontos não colineares como mostrado na Figura \ref{fig:lateration}. Nesta figura, para se obter a localização da entidade X é necessário obter a distância entre a mesma e três pontos de referência não colineares.
		...
		\begin{figure}[htb]
			...
			\end{center}
			\caption{ (a) Exemplo de Lateração. Adaptada de \cite{triangulacao}.
			(b) Exemplo de Angulação. Adaptada de ~\cite{triangulacao}.}
		\end{figure}
		...
		método de \textit{Stereo Vision}, porém são muito caras e pouco
		...
		\begin{figure}[H]
			...
			pontos de luz infravermelha utilizando o método Luz Estruturada~\cite{img-strutuctured-light}.}
